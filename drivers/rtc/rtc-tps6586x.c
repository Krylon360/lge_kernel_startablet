...
#define CHECK_RTC_SET    // To check if rtc set is correct or not
...
#ifdef CHECK_RTC_SET
	tps6586x_reads(tps_dev, RTC_ALARM2_LO, sizeof(temp), &temp);
	tps6586x_reads(tps_dev, RTC_ALARM2_HI, sizeof(temp), &temp);
#endif
...
#ifdef CHECK_RTC_SET
	printk(KERN_INFO "time read as %lu. %d/%d/%d %d:%02u:%02u\n",
		seconds,
		tm->tm_mon+1,
		tm->tm_mday,
		tm->tm_year+1900,
...
static int tps6586x_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
...
#ifdef CHECK_RTC_SET
	printk(KERN_INFO "time set to %lu. %d/%d/%d %d:%02u:%02u\n",
		seconds,
		tm->tm_mon+1,
		tm->tm_mday,
		tm->tm_year+1900,
...
#ifdef CHECK_RTC_SET
	/** read time to verify **/
	err = tps6586x_reads(tps_dev, RTC_COUNT4, ARRAY_SIZE(buff_read), buff_read);
	if (err < 0) {
		dev_err(dev, "failed to read counter\n");
		return err;
	}
	...
	tps6586x_reads(tps_dev, RTC_ALARM2_LO, sizeof(temp), &temp);
	tps6586x_reads(tps_dev, RTC_ALARM2_HI, sizeof(temp), &temp);
	tps6586x_reads(tps_dev, RTC_CTRL, sizeof(temp), &temp);

	printk(KERN_NOTICE "@@@ set buff[0,1,2,3,4] =  %2X  %2X  %2X  %2X  %2X\n",
		buff[0], buff[1], buff[2], buff[3], buff[4]);

	printk(KERN_NOTICE"@@@ read buff[0,1,2,3,4] = %2X  %2X  %2X  %2X  %2X\n",
		buff_read[0], buff_read[1], buff_read[2], buff_read[3], buff_read[4]);

	if (buff[0] != buff_read[0] || buff[1] != buff_read[1] || buff[2] != buff_read[2]
		|| buff[3] != buff_read[3]/* || buff[4] != buff_read[4]*/)
	{
		printk(KERN_ERR "@@@ ERROR : set and read value are different!!!");
	}
#endif
...
#ifdef CHECK_RTC_SET
	printk(KERN_INFO "alarm set as %lu. %d/%d/%d %d:%02u:%02u\n",
		seconds,
		alrm->time.tm_mon+1,
		alrm->time.tm_mday,
