...
void echo_set_bypass_parameters()
{
	...
	int err = 0;
	if ( echo_read_register(0x2308) != 0x005F) err = 1;
	if ( echo_read_register(0x232C) != 0x0025) err = 1;
	if ( echo_read_register(0x23F8) != 0x4003) err = 1;
	if ( echo_read_register(0x230C) != 0x5A5A) err = 1;
	...
	if (ret != 0) {		//get the lock
		do {
			...
			if (err !=0) printk("\n[FM31] %d :  bypass parameter write fail\n",i);
			if (rval != 0x5a5a) printk("0x230C return value = %x, count = %d \n", rval, i);
			i++;
		} while ((err != 0) || (rval != 0x5a5a));
		msleep(1);
		gpio_set_value(GPIO_ECHO_BP_N,0);
...
void echo_set_parameters()
{
	...
	if (ret != 0) {		//get the lock
		...
		do {
			...
			msleep(1);
			rval = echo_read_register(0x230C);
			if (err !=0) printk("\n[FM31] %d :  voip parameter write fail\n",i);
			i++;
		} while ((err != 0) | (rval != 0x5a5a));
...
void echo_set_headset_parameters()
{
	...
	if (ret != 0) {		//get the lock
		...
		do {
			...
			msleep(1);
			rval = echo_read_register(0x230C);
			if (err !=0) printk("\n[FM31] %d :  voip headset parameter write fail\n",i);
			i++;
		} while ((err != 0) | (rval != 0x5a5a));
