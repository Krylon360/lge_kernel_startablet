...
u16 noise_filtering_level_change_worker(struct work_struct *work)
{
	...
	mxt = container_of(work, struct mxt_data, cwork.work);
	client = mxt->client;
	...
	tmp_1 = (uint8_t *) kmalloc(object_size, GFP_KERNEL);
	...
	*(tmp_1 + 0) = MXT_T22_FREQHOPSCALE_IDLE;

	ret = mXT1386_i2c_write(client, object_address_1, tmp_1, object_size);

	/* Back_up Config */
	tmp_2 = (uint8_t *) kmalloc(object_size, GFP_KERNEL);
	...
	*(tmp_2 + 0) = 0x55;
	...
	ret = mXT1386_i2c_write(client, object_address_2, tmp_2, object_size);

	kfree(tmp_1);
	kfree(tmp_2);

	for(i = 0; i < 10; i++)
		mxt_read_block(client, 155, 9, message);
...
param_check_ok:
	...
	INIT_DELAYED_WORK(&mxt->cwork, noise_filtering_level_change_worker);

	schedule_delayed_work(&mxt->cwork, 2000);
