...
static bool tegra_dc_hdmi_detect(struct tegra_dc *dc)
{
	...
	old_level = tegra_dc_hdmi_hpd(dc);
	/* wait until hpd pin voltage level is stable */
	while (count < 3) {
		msleep(100);
		level = tegra_dc_hdmi_hpd(dc);
		if(old_level == level)
			count++;
		else
			count = 0;
	...
	/* go to the routine that handles unplugging action */
	if (!level)
		goto fail;

	/* reaching here if hpd is 1, but skip the routine that handles the
	   plugging action if hdmi is aleady plugged in order to remove
	   unnessasary operation */
	if(hdmi->hpd_switch.state == 1)
		return true;
#else
...
#if defined (CONFIG_MACH_STARTABLET)
	/* reaching here if hpd is 0, but skip the routine that handles the
	   unplugging action if hdmi is aleady unplugged in order to remove
	   unnessasary operation */
	if(hdmi->hpd_switch.state == 0)
		return false;
...
#if !defined (CONFIG_MACH_STARTABLET)
		/* this routine moves in the tegra_dc_hdmi_detect */
		tegra_dc_disable(dc);
		tegra_fb_update_monspecs(dc->fb, NULL, NULL);
#endif
	}
